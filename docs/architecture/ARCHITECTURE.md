...
| **Twenty CRM image**           | ✅ Supported | ✅ Supported   | Multi-arch Docker image                                                                         |
...
## 8. Application Profile Schema - 11-Field Specification

### Profile Schema Fields

| #   | Field             | Obligation | Type                                          | Description                                                                                                                                                                              |
| --- | ----------------- | ---------- | --------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | `name`            | **MUST**   | string                                        | Application name. Must match the profile directory name.                                                                                                                                 |
| 2   | `version`         | **MUST**   | string                                        | Pinned version or Docker image tag (e.g., `"4.12.1"`, `"latest"`). Version pinning required for production; `latest` accepted for dev.                                                   |
| 3   | `compose_file`    | **MUST**   | string (path)                                 | Path to the Jinja2 compose template, relative to the profile directory (e.g., `compose.yml.j2`).                                                                                         |
| 4   | `vars_file`       | **MUST**   | string (path)                                 | Path to `vars.yml` containing all configurable parameters (e.g., port, DB name, resource limits). Formerly `vars`.                                                                       |
| 5   | `secrets_file`    | **MUST**   | string (path)                                 | Path to secrets manifest sourced from Ansible Vault (e.g., `secrets.yml`). Contains required secret key names.                                                                           |
| 6   | `supported_arch`  | **MUST**   | array of strings                              | List of supported architectures, e.g., `["amd64", "arm64"]`. Playbook refuses deployment if host arch is not in this list.                                                               |
| 7   | `depends_on`      | SHOULD     | array of objects                              | Dependencies on other roles or apps. Each entry has `role` (string). Formalizes cross-app dependencies. Example: `[{role: "L4_networking/caddy"}, {role: "backup"}, {role: "observability"}]`. |
| 8   | `backup`          | SHOULD     | object                                        | Backup policy for app data (L5). Fields: `schedule` (cron), `retention_daily`, `retention_weekly`, `retention_monthly`, `db_type` (postgres/mysql/mongodb), `db_name`.                   |
| 9   | `monitoring`      | SHOULD     | object                                        | Monitoring targets (L3). Fields: `health_endpoint` (path), `health_port` (int), `scrape_port` (int).                                                                                     |
| 10  | `compliance_tags` | SHOULD     | array of strings                              | Compliance control tags using standard prefix: `nist,ac-2`, `cis,1.1`, `soc2,cc6.1`. Never generic tags like `compliance,ac-2`.                                                          |
| 11  | `dr_tier`         | SHOULD     | enum: `critical` / `standard` / `best-effort` | Disaster recovery priority tier. Determines backup frequency, retention, and restore SLAs.                                                                                               |

### Validation Rules

1. **MUST fields**: `name`, `version`, `compose_file`, `vars_file`, `secrets_file`, `supported_arch` - all must be present and non-empty.
2. **Path validation**: `compose_file`, `vars_file`, and `secrets_file` must reference files that exist in the profile directory.
3. **`supported_arch`**: Must be a non-empty array. Valid values: `"amd64"`, `"arm64"`.
4. **Unknown fields**: The validator MUST reject any field not in the schema.
5. **`backup.db_type`**: If `backup` is present, `db_type` must be one of: `postgres`, `mysql`, `mariadb`, `mongodb`, `sqlite`.
6. **`monitoring.health_endpoint`**: If `monitoring` is present, must start with `/`.
7. **`dr_tier`**: If present, must be one of: `critical`, `standard`, `best-effort`.
8. **Pre-flight assertion** (at deploy time): `apps.yml` asserts that all required secrets (from `secrets_file`) exist in the resolved Vault context before any container is started.

### Example: Chatwoot Profile

```yaml
# apps/chatwoot/profile.yml
name: chatwoot
version: "4.12.1"
compose_file: compose.yml.j2
vars_file: vars.yml
...
monitoring:
  ...
  health_port: 3000
  scrape_port: 3000
compliance_tags: [ac-2, sc-7, si-4, au-12]
dr_tier: critical
```
...
| #   | App Name            | Description                                                 | DB Type                          | Backup Tier | Port Map (host:container) | Custom Image              |
| --- | ------------------- | ----------------------------------------------------------- | -------------------------------- | ----------- | ------------------------- | ------------------------- |
| 1   | **Chatwoot**        | Open-source customer support platform                       | PostgreSQL                       | Critical    | 3000:3000                 | ❌ Official multi-arch    |
| 2   | **n8n**             | Fair-code workflow automation                               | PostgreSQL                       | Critical    | 5678:5678                 | ❌ Official multi-arch    |
| 3   | **Twenty CRM**      | Modern open-source CRM                                      | PostgreSQL                       | Critical    | 3001:3000                 | ❌ Official multi-arch    |
| 4   | **OpenWebUI**       | Self-hosted AI chat interface                               | PostgreSQL (or SQLite)           | Standard    | 8080:8080                 | ❌ Official multi-arch    |
| 5   | **Metabase**        | Business intelligence and analytics                         | PostgreSQL (application-managed) | Standard    | 3002:3000                 | ❌ Official multi-arch    |
| 6   | **NocoDB**          | Open-source Airtable alternative (database-spreadsheet)     | PostgreSQL / MySQL / SQLite      | Standard    | 8081:8080                 | ❌ Official multi-arch    |
| 7   | **FastAPI Backend** | Custom FastAPI backend (example/template for user projects) | PostgreSQL (configurable)        | Best-effort | 8000:8000                 | ⚠️ **Community-provided** |
...
| ------------------------------------ | -------------------------------------------------------- | ----------------------------------------------------------- | ---------------------------------------------------------------------- |
| `compose_file`                       | Render `.j2` template → `docker compose up`              | Ignored - Swarm uses stack YAML with different directives   | Ignored - K3s uses Kubernetes manifests                                |
| `vars_file` + `secrets_file`         | Vault → `.env` file mounted in container                 | Vault → Docker secrets (`docker secret create`)             | Vault → Kubernetes Secrets (base64-encoded, mounted as volumes or env) |
| `backup.db_type` + `backup.schedule` | `pg_dump` / `mysqldump` container with cron schedule     | `pg_dump` / `mysqldump` pod with CronJob equivalent         | Kubernetes CronJob with persistent volume claims                       |
| `monitoring.health_endpoint`         | HTTP GET check via VictoriaMetrics/Grafana scrape config | HTTP GET check (same)                                       | Kubernetes liveness/readiness probe from `health_endpoint`             |
...
- `compose_file` is a **path hint**, not a runtime switch. Swarm/K3s adapters ignore it.
- `vars_file` and `secrets_file` declare **what** variables and secrets are needed, not **how** they are injected.
- `backup` declares the **data** to protect (db_type, db_name), not the backup mechanism.
